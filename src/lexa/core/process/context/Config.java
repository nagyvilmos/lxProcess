/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */

package lexa.core.process.context;

/**
 *
 * @author William
 * @since YYYY-MM
 */
public class Config {
    /** "{@code allowAnonymous}" */
    public static final String ALLOW_ANONYMOUS
            = "allowAnonymous";
    /** "{@code loader}" */
    public static final String CALCULATION
            = "calculation";
    /** "{@code loader}" */
	public static final String CLASS_LOADER
            = "loader";
    /** "{@code classPath}" */
    public static final String CLASS_PATH
            = "class";
    /** "{@code config}" */
    public static final String CONFIG
            = "config";
    /** "{@code connectionName}" */
    public static final String CONNECTION_NAME
            = "connectionName";
    /** "{@code data}" */
    public static final String DATA
            = "data";
    /** "{@code fields}" */
    public static final String REQUEST_FIELD_LIST
            = "requestFieldList";
    /** "{@code handleRequest}" */
    public static final String REQUEST_EXPRESSION
            = "requestExpression";
    /** "{@code logging}" */
    public static final String LOGGING
            = "logging";
    /** "{@code maxProcesses}" */
    public static final String MAX_PROCESSES
            = "maxProcesses";
    /** "{@code name}" */
    public static final String NAME
            = "name";
    /** "{@code processList}" */
    public static final String PROCESS_LIST
            = "processList";
   /** "{@code serviceList}" */
    public static final String SERVICE_LIST
            = "serviceList";
    /** "{@code replyMap}" */
    public static final String REPLY_MAP
            = "replyMap";
    /** "{@code timeout}" */
    public static final String TIMEOUT
            = "timeout";
    /** "{@code wildcard}" */
    public static final String WILDCARD
            = "wildcard";

    public static final String REQUEST_LIST     = "requestList";
    public static final String NEXT_REQUEST     = "nextRequest";
    public static final String HOST             = "host";
    public static final String PORT             = "port";
    public static final String BROKER_LIST      = "brokerList";
    public static final String LOG_FILE         = "logFile";
	public static final String TYPE				= "type";
}
